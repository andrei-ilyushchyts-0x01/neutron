...
| `binder_call`             | `bool`     | event is a `type:"binder_call"` synthesised pair         |
| `binder_status_in`        | `[string]` | `status` ∈ list (`completed`, `callee_crashed`, ...)     |
| `binder_code_in`          | `[u32]`    | AIDL transaction `code` ∈ list                           |
...
The `binder_*` predicates only match `type:"binder_call"` events (sprint-2
PR 2). These are synthesised by the userspace correlator from paired
`binder_transaction` (caller) / `binder_transaction_received` (callee)
events; the rule engine maps `caller_pid` → `pid` so `aggregate: per_process`
collapses calls from the same caller. `binder_status_in` accepts
`completed` (pair matched), `callee_crashed` (callee died with the
transaction in flight), and `unmatched` (LRU-evicted; not currently
emitted by default).
...
### Binder-causality example (R004)

Sprint-2 PR 2 introduced `binder_call` events. They are caller↔callee pairs
with a lifecycle status. R004 fires once per caller PID when a callee
...
To narrow to a specific service interface (e.g. only SurfaceFlinger), add
`binder_code_in: [<aidl_codes>]`. The `code` value is the AIDL transaction
ordinal — read it from the autogenerated `Transaction` constants in the
service's `.aidl` interface.
