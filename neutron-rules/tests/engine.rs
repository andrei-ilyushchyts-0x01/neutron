...
fn load_dexprotector_engine() -> RuleEngine {
    ...
    let rules = neutron_rules::load_rules_yaml_file(&path)
        .unwrap_or_else(|err| panic!("DexProtector ruleset should load from {}: {err}", path.display()));
...
fn default_ruleset_loads() {
    ...
    assert!(engine.rule_count() >= 15, "expected >=15 default rules, got {}", engine.rule_count());
}
...
fn proc_self_maps_polling_fires_after_threshold() {
    ...
    let hit: Vec<_> = findings
        ...
        .collect();
    assert_eq!(hit.len(), 1, "expected exactly one T001 finding, got {}: {findings:?}", hit.len());
...
fn single_proc_self_maps_does_not_fire() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T001_proc_self_maps_polling");
    assert!(hit.is_none(), "T001 should NOT fire on a single event: {findings:?}");
}
...
fn su_binary_probe_fires_immediately() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T004_su_binary_probe");
    assert!(hit.is_some(), "T004 should fire on first faccessat to /system/xbin/su: {findings:?}");
}
...
fn su_binary_probe_fires_on_statfs() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T004_su_binary_probe");
    assert!(hit.is_some(), "T004 should fire on statfs to /system/xbin/su: {findings:?}");
}
...
fn rwx_mmap_fires_per_event() {
    ...
    let findings = engine.drain_ready();
    let hits: Vec<_> = findings.iter().filter(|f| f.rule_id == "T011_rwx_memory_allocation").collect();
    assert_eq!(hits.len(), 2, "T011 with every_event should emit twice for two RWX mmaps");
}
...
fn frida_artifact_probe_per_target_only_emits_once_per_path() {
    ...
    let findings = engine.drain_ready();
    let hits: Vec<_> = findings.iter().filter(|f| f.rule_id == "T006_frida_artifact_probe").collect();
    assert_eq!(hits.len(), 2, "expected one finding per distinct target, got {hits:?}");
}
...
fn t015_does_not_fire_on_proc_self_maps() {
    ...
    let findings = engine.drain_ready();
    let t015 = findings.iter().find(|f| f.rule_id == "T015_cross_process_proc_inspection");
    assert!(t015.is_none(), "T015 must NOT fire on /proc/self/maps: {findings:?}");
}
...
fn t015_fires_on_cross_process_inspection() {
    ...
    let findings = engine.drain_ready();
    let t015 = findings.iter().find(|f| f.rule_id == "T015_cross_process_proc_inspection");
    assert!(t015.is_some(), "T015 should fire on /proc/<other_pid>/maps: {findings:?}");
}
...
fn stack_contains_matches_when_substring_present() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T016_native_root_check_via_libc");
    assert!(hit.is_some(), "T016 should fire when stack contains 'libc': {findings:?}");
}
...
fn stack_contains_does_not_match_when_stack_absent() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T016_native_root_check_via_libc");
    assert!(hit.is_none(), "T016 must NOT fire without stack data: {findings:?}");
}
...
fn stack_not_contains_excludes_renderscript() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T019_native_lib_path_inspection");
    assert!(hit.is_none(), "T019 must NOT fire when stack mentions RenderScript: {findings:?}");
}
...
fn stack_not_contains_passes_when_stack_absent() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T019_native_lib_path_inspection");
    assert!(hit.is_some(), "T019 should fire on /system/lib64/* with no stack: {findings:?}");
}
...
fn t017_jit_cache_fires_after_threshold() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T017_jit_cache_syscall");
    assert!(hit.is_some(), "T017 should fire after 5+ JIT-frame syscalls in 10s: {findings:?}");
}
...
fn t020_anon_mapping_origin_fires_for_banking_app_pattern() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T020_native_check_from_anon_mapping");
    assert!(hit.is_some(), "T020 should fire on /proc/self/maps from [anon: stack: {findings:?}");
}
...
fn t020_does_not_fire_when_stack_is_libc_only() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T020_native_check_from_anon_mapping");
    assert!(hit.is_none(), "T020 must NOT fire without [anon: in stack: {findings:?}");
}
...
fn t021_frida_thread_comm_scan_fires_after_threshold() {
    ...
    feed_lines(&mut engine, &line_refs);
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T021_frida_thread_comm_scan");
    assert!(hit.is_some(), "T021 should fire after 5+ task/<TID>/comm reads: {findings:?}");
}
...
fn t021_does_not_fire_for_unrelated_proc_self_paths() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T021_frida_thread_comm_scan");
    assert!(hit.is_none(), "T021 must NOT fire on /proc/self/{{maps,status}}: {findings:?}");
}
...
fn t022_bpf_syscall_from_app_fires() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T022_unexpected_bpf_syscall");
    assert!(hit.is_some(), "T022 should fire for bpf() from a regular app: {findings:?}");
}
...
fn prctl_pr_get_dumpable_fires() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T010_prctl_dumpable_check");
...
fn dp001_dexprotector_boot_library_probe_fires() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "DP001_dexprotector_boot_libraries");
    assert!(hit.is_some(), "DP001 should fire on libdexprotector.so access: {findings:?}");
}
...
fn dp002_dexprotector_asset_blob_probe_fires() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "DP002_dexprotector_asset_blobs");
    assert!(hit.is_some(), "DP002 should fire on DexProtector asset blob access: {findings:?}");
}
...
fn dp004_dexprotector_proc_maps_rasp_fires_with_native_stack() {
    ...
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "DP004_dexprotector_proc_maps_rasp");
    assert!(hit.is_some(), "DP004 should fire on /proc/self/maps from libdp stack: {findings:?}");
}
...
fn t001_finding_carries_schema_v2_fields_through_engine() {
    ...
    assert!(
        hit.interpretation.iter().any(|s| s.contains("anti-instrumentation")),
        "expected anti-instrumentation interpretation, got {:?}",
        hit.interpretation
    );
    assert_eq!(hit.confidence, Some(0.85));
    assert!(
        hit.false_positives.iter().any(|fp| fp.contains("crash reporters")),
...
fn finding_json_includes_v2_fields_when_set_on_rule() {
    ...
    feed_lines(&mut engine, &lines);
    let findings = engine.drain_ready();
    let hit = findings.iter().find(|f| f.rule_id == "T001_proc_self_maps_polling").unwrap();
...
fn finding_json_omits_v2_fields_when_unset() {
    ...
    let s = serde_json::to_string(hit).unwrap();
    assert!(!s.contains(r#""behavior":"#), "T002 has no v2 behavior; JSON: {s}");
    assert!(!s.contains(r#""confidence":"#), "T002 has no v2 confidence; JSON: {s}");
    // false_positives and interpretation default to empty Vec, omitted by skip rule.
    assert!(!s.contains(r#""interpretation":["#), "expected omitted interpretation; got {s}");
}
